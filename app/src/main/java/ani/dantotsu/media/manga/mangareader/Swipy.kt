...
class Swipy @JvmOverloads constructor(
    ...
    // Callbacks
    var onSwipeTop: ((Float) -> Unit)? = null
    var onSwipeBottom: ((Float) -> Unit)? = null
    var onSwipeLeft: ((Float) -> Unit)? = null
    var onSwipeRight: ((Float) -> Unit)? = null
    ...
    private fun handleDrag(delta: Float) {
        ...
        when (swipeDirection) {
            SwipeDirection.TOP -> onSwipeTop?.invoke(progress)
            SwipeDirection.BOTTOM -> onSwipeBottom?.invoke(progress)
            SwipeDirection.LEFT -> onSwipeLeft?.invoke(progress)
            SwipeDirection.RIGHT -> onSwipeRight?.invoke(progress)
            SwipeDirection.NONE -> {}
        }
    }
    ...
    private fun finishDrag(distance: Float) {
        ...
        if (abs(distance) > threshold) {
            when (swipeDirection) {
                SwipeDirection.TOP -> onSwipeTopComplete?.invoke()
                SwipeDirection.BOTTOM -> onSwipeBottomComplete?.invoke()
                SwipeDirection.LEFT -> onSwipeLeftComplete?.invoke()
                SwipeDirection.RIGHT -> onSwipeRightComplete?.invoke()
                SwipeDirection.NONE -> {}
            }
        }
    }
    ...
    private fun reset() {
        ...
        onSwipeTop?.invoke(0f)
        onSwipeBottom?.invoke(0f)
        onSwipeLeft?.invoke(0f)
        onSwipeRight?.invoke(0f)
    }
